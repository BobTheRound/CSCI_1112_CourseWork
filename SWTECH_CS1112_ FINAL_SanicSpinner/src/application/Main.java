...
public class Main extends Application {
	...
	public void start(Stage primaryStage) {
		try {
			...
			Button buttStop = new Button("||");
			Button buttGo = new Button(">>");
			
			HBox buttCarrier = new HBox();
			VBox radButtHolder = new VBox();
			...
			//set pause button
			buttStop.setOnMousePressed(e -> {
				gottaGoFast.stop();
				counter.stop();
			});
			
			//set go button
			buttGo.setOnMousePressed(e -> {
				gottaGoFast.play();
				counter.play();
			});
			...
			radButtHolder.getChildren().addAll(zFlip, xFlip, yFlip);
			radButtHolder.setAlignment(Pos.TOP_LEFT);
			root.setLeft(radButtHolder);
			
			buttCarrier.getChildren().addAll(buttStop, buttGo);
			buttCarrier.setAlignment(Pos.TOP_CENTER);
			buttCarrier.getChildren().add(coolSlide);
			
			root.setBottom(buttCarrier);
			
			BorderPane.setAlignment(buttCarrier, Pos.BOTTOM_CENTER);
